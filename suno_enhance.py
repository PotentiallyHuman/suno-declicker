...
def show_comparison(orig, enh, sr, original_path, enhanced_path):
    ...
    # ── Audio preview ─────────────────────────────────────────────────────────
    if not _have_sd:
        print("  (sounddevice not installed — skipping audio preview)")
        return

    print("\n  [Comparison] Detecting click positions for audio preview...")
    clicks = detect_clicks(orig, sr)
    if not clicks:
        print("  No clicks detected — nothing to preview.")
        return

    # Sort by peak amplitude in the original (loudest = most audible to a listener)
    def _peak(click):
        _, s, e, *_ = click
        return float(np.abs(orig[s:e + 1]).max()) if e > s else 0.0

    top = sorted(clicks, key=_peak, reverse=True)[:5]
    pad = 2 * sr   # 2 s each side
    ch  = orig.shape[1]

    print(f"\n  Playing top {len(top)} click regions — original then declicked.")
    print(f"  Press Enter between each, or Ctrl+C to stop.\n")

    for i, (centre, start, end, fsim, *_) in enumerate(top):
        t       = centre / sr
        m, sec  = divmod(t, 60)
        seg_s   = max(0, centre - pad)
        seg_e   = min(n, centre + pad)

        print(f"  ── Click {i + 1}/{len(top)}  at {int(m):02d}:{sec:05.2f}  "
              f"(match={fsim:.3f}) ──")
        try:
            print(f"    ▶  Original  [{seg_s/sr:.1f}s – {seg_e/sr:.1f}s]")
            sd.play(orig[seg_s:seg_e].copy(), sr, blocking=True)
            sd.sleep(400)

            print(f"    ▶  Declicked [{seg_s/sr:.1f}s – {seg_e/sr:.1f}s]")
            sd.play(enh[seg_s:seg_e].copy(), sr, blocking=True)
            sd.sleep(400)

            if i < len(top) - 1:
                input("    [Enter for next click]\n")
        except KeyboardInterrupt:
            sd.stop()
            print("\n  Stopped early.\n")
            break

    print("  Audio preview complete.\n")
...
def _interactive():
    ...
    print("    - Full song mix")
    print("    - Wet vocal stem with effects   (optional, improves high-freq repair)")
    print("    - Instrumental stem             (optional, improves high-freq repair)")
    print("    - Dry vocal stem (no effects)   (optional, catches longer artifacts)")
    ...
    wet_vocal = None
    instrumental = None
    if _yn("Do you have vocal + instrumental stems for better click repair?"):
    ...
    dry_vocal = None
    if _yn("Do you have a dry vocal stem (no effects) to verify long artifacts?"):
        dry_vocal = _pick("Dry vocal stem WITHOUT effects:", files, session_dir, exclude=used)
    ...
    if instrumental: print(f"  Instrumental : {os.path.basename(instrumental)}")
    if dry_vocal:   print(f"  Dry vocal    : {os.path.basename(dry_vocal)}")
    input("\n  Press Enter to start, or Ctrl+C to cancel. ")

    return original, wet_vocal, instrumental, dry_vocal, session_dir
...
def main():
    ...
    if not args.input:
        (args.input, wet_vocal, args.instrumental,
         dry_vocal, session_dir) = _interactive()
    ...
    if not args.no_compare:
        ans = input("  Show spectrogram comparison + audio preview? (y/N): ").strip().lower()
        if ans == 'y':
